...
export function SearchFilters({ stats }: { stats: StatsResponse }) {
  ...
  const [naicsCode, setNaicsCode] = useState(searchParams.get("naics_code") ?? "");
  ...
  const applyFilters = useCallback(() => {
    ...
    router.push(qs ? `/?${qs}` : "/");
  }, [search, naicsCode, oppType, setAside, state, department, activeOnly, router]);
  ...
  // Top items for dropdowns (max 50)
  ...
  return (
    <div className="space-y-4">
      ...
      <div>
        ...
        <Select value={naicsCode || ALL_VALUE} onValueChange={(v) => setNaicsCode(v === ALL_VALUE ? "" : v)}>
          <SelectTrigger>
            <SelectValue placeholder="All NAICS" />
          </SelectTrigger>
          <SelectContent>
            <SelectItem value={ALL_VALUE}>All NAICS</SelectItem>
            {topNaics.map((n) => (
